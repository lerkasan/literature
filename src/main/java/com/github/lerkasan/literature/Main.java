...
public class Main {
	...
	/*	RssService rssService = new RssServiceImpl();
		...
		Resource dzone = resource.getByName("DZone");
		if (dzone != null) {
			List<SyndEntry> rssNews = rssService.read(dzone);
	...
	/*	Resource crossrefApi = new Resource();
		crossrefApi.setId(1);
		 ...
		 * = new BufferedReader(new FileReader("result.txt"))) { String l; while
		 * ((l = inputStream.readLine()) != null) { sb.append(l); } } catch

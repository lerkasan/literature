...
public class SpringerParsingServiceImpl extends ParsingServiceImpl implements ParsingService {
	...
	/*
	 * @Inject private ItemToReadService itemToReadService;
	 * 
	...
	/*
	 * public Literature convertToItem(SpringerApiJson item) { Literature
	 * literatureItem = new Literature();
	 ...
	 * literatureItem.setUrl(item.getUrl()); if ((item.getPublicationDate() !=
	 * null) && (item.getPublicationDate() != "")) { DateTimeFormatter formatter
	 * = DateTimeFormatter.ofPattern("yyyy-MM-dd"); LocalDate date =
	 * LocalDate.parse(item.getPublicationDate(), formatter);
	...
	/*
	 * public String save(int[] selectedItemsIds, List<ConvertableToItemToRead>
	 * items) {
	 * 
	 * boolean oldItemsFlag = false; if ((selectedItemsIds != null) && (items !=
	 * null)) { for (int i : selectedItemsIds) { ConvertableToItemToRead
	 * convertableItem = items.get(i); String url = convertableItem.getUrl();
	 * ItemToRead foundItem = itemToReadService.getByUrl(url); if (foundItem ==
	 * null) { ItemToRead item = convertableItem.convertToItem();
	 * item.setAddedAt(LocalDate.now()); itemToReadService.save(item); } else {
	 * oldItemsFlag = true; } } } return oldItemsFlag ?
	 * Messages.SOME_ITEMS_ALREADY_IN_DB : Messages.SUCCESSFUL_SAVE; }
	 */
